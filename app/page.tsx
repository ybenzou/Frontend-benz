...
export default function Overview() {
  ...
  return <div className="page-grid pb-16 md:pb-0">
    <div className="flex items-end justify-between gap-6">
      <div><p className="eyebrow">Institutional dashboard · {overview.snapshotDate}</p><h1 className="page-title">Market overview</h1></div>
    ...
    <section className="index-grid panel overflow-hidden">
      {overview.indices.map((item, index) => <div key={item.symbol} className={`min-h-[112px] border-b border-r border-[var(--line-subtle)] p-4 ${index === 0 ? "bg-[var(--surface-raised)] sm:p-5" : ""}`}>
        <div className="flex items-center justify-between"><span className="muted text-[11px]">{item.name}</span><span className="eyebrow">{item.symbol}</span></div>
        <div className={`tnum mt-3 font-semibold tracking-[-.045em] ${index === 0 ? "text-[28px]" : "text-[24px]"}`}>{item.value}</div>
        <div className={`tnum mt-1 text-[11px] ${item.change >= 0 ? "positive" : "negative"}`}>{formatPercent(item.change)} <span className="font-sans tracking-normal text-[var(--muted)]">session</span></div>
      </div>)}
    </section>
    <div className="overview-grid">
      ...
      <section className="overview-secondary panel">
        ...
        <div className="grid grid-cols-2">
          {overview.sectors.map(([name, change]) => <div key={name} className="border-b border-r border-[var(--line-subtle)] p-3"><div className="muted text-[11px]">{name}</div><div className={`tnum mt-1.5 font-medium ${change >= 0 ? "positive" : "negative"}`}>{formatPercent(change)}</div><div className={`sector-bar mt-2 h-[2px] ${change >= 0 ? "text-[var(--positive)]" : "text-[var(--negative)]"}`}><span style={{ width: `${Math.min(Math.abs(change) * 55, 100)}%` }}/></div></div>)}
        </div>
      </section>
      <section className="overview-secondary panel">
        <div className="panel-header"><span className="section-label">Session events</span><CalendarClock size={15} className="muted"/></div>
        <div>{overview.events.map((event) => <div key={event.time} className="grid grid-cols-[42px_45px_1fr] items-start gap-2 border-b border-[var(--line-subtle)] px-3 py-2.5 last:border-0"><span className="tnum muted text-[10px]">{event.time}</span><span className="eyebrow text-[var(--accent)]">{event.tag}</span><div><div className="text-xs font-medium">{event.title}</div><div className="muted mt-0.5 line-clamp-1 text-[10px]">{event.detail}</div></div></div>)}</div>
      </section>
    </div>
  </div>;
}
