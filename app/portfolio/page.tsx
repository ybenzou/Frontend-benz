  ...
  return <div className="page-grid pb-16 md:pb-0">
    ...
    <div className="desktop-grid grid grid-cols-[minmax(0,1.6fr)_minmax(280px,.6fr)] gap-3">
      ...
      <div className="page-grid">
        <section className="panel"><div className="panel-header"><span className="font-semibold">Allocation</span><span className="eyebrow">Derived sector</span></div><div className="p-4"><div className="mb-5 flex h-3 overflow-hidden" role="img" aria-label={summary.allocations.map((item) => `${item.sector} ${item.weight.toFixed(1)}%`).join(", ")}>{summary.allocations.map((item) => <div title={item.sector} key={item.sector} style={{ width: `${item.weight}%`, background: allocationColors[item.sector] ?? "#8b98a9" }}/>)}</div><div className="space-y-3">{summary.allocations.map((item) => <div className="flex items-center" key={item.sector}><span className="mr-2 size-2" style={{ background: allocationColors[item.sector] ?? "#8b98a9" }}/><span className="muted flex-1">{item.sector}</span><strong className="tnum">{item.weight.toFixed(1)}%</strong></div>)}</div></div></section>
        <section className="panel"><div className="panel-header"><span className="font-semibold">Recent activity</span><span className="eyebrow">Mock</span></div><div className="divide-y divide-[#202a35]">{portfolio.events.map((event) => <div className="flex items-center gap-3 p-3" key={event.title}><span className="eyebrow w-8 text-[#76a7ff]">{event.tag}</span><span className="flex-1 text-xs">{event.title}</span><span className="muted tnum text-xs">{event.time.slice(5)}</span></div>)}</div></section>
      </div>
    </div>
  </div>;
}
