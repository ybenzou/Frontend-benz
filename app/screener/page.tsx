...
export default function ScreenerPage() {
  ...
  return <div className="page-grid pb-16 md:pb-0">
    <div className="flex items-end justify-between"><div><p className="eyebrow">Equity tools · Mock universe</p><h1 className="mt-1 text-xl font-semibold">Stock screener</h1></div><button disabled title="Saving screens is unavailable in this demo" aria-label="Save screen unavailable in demo" className="control flex items-center gap-2"><Save size={14}/> Save screen</button></div>
    <section className="panel">
      <div className="panel-header"><span className="font-semibold">Criteria</span><button onClick={() => { setSector("All sectors"); setCap(0); setMaxPe(80); }} className="muted flex items-center gap-1 text-xs hover:text-white"><RotateCcw size={13}/> Reset</button></div>
      <div className="grid gap-4 p-4 sm:grid-cols-3">
        ...
        <label><span className="eyebrow mb-2 flex justify-between">Maximum P/E <b className="tnum text-white">{maxPe}x</b></span><input className="w-full accent-[#4e8cff]" type="range" min="10" max="80" value={maxPe} onChange={(e) => setMaxPe(Number(e.target.value))}/></label>
      </div>
    </section>
    <section className="panel overflow-hidden"><div className="panel-header"><span className="font-semibold">Results</span><span className="muted text-xs">{rows.length} companies · sorted by market cap</span></div><div className="overflow-x-auto"><table className="data-table tnum"><thead><tr><th>Company</th><th>Sector</th><th>Price</th><th>1D</th><th>Market cap</th><th>P/E</th><th>Volume</th></tr></thead><tbody>
