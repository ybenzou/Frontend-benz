...
const tooltipStyle = { background: palette.surface, border: `1px solid ${palette.line}`, borderRadius: 4, fontSize: 12 };
...
export function PriceChart({ data }: { data: { date: string; price: number }[] }) {
  ...
  return <div className="h-[330px] w-full" role="img" aria-label={summary}><ResponsiveContainer width="100%" height="100%"><AreaChart data={data} margin={{ top: 20, right: 14, bottom: 0, left: 4 }}><defs><linearGradient id="priceFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={palette.accent} stopOpacity={0.16}/><stop offset="100%" stopColor={palette.accent} stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke={palette.line} vertical={false}/><XAxis dataKey="date" tick={{ fill: palette.muted, fontSize: 10 }} axisLine={false} tickLine={false}/><YAxis domain={["dataMin - 8", "dataMax + 5"]} orientation="right" tick={{ fill: palette.muted, fontSize: 10 }} axisLine={false} tickLine={false}/><Tooltip contentStyle={tooltipStyle}/><Area type="monotone" dataKey="price" stroke={palette.accent} fill="url(#priceFill)" strokeWidth={2}/></AreaChart></ResponsiveContainer></div>;
}

export function FinancialChart({ data }: { data: { year: string; revenue: number; income: number }[] }) {
  const latest = data.at(-1);
  return <div className="h-52 w-full" role="img" aria-label={`Mock financial trend; latest revenue ${latest?.revenue} billion and income ${latest?.income} billion`}><ResponsiveContainer width="100%" height="100%"><BarChart data={data}><CartesianGrid stroke={palette.line} vertical={false}/><XAxis dataKey="year" tick={{ fill: palette.muted, fontSize: 10 }} axisLine={false}/><YAxis tick={{ fill: palette.muted, fontSize: 10 }} axisLine={false}/><Tooltip contentStyle={tooltipStyle}/><Bar dataKey="revenue" fill={palette.accent}/><Bar dataKey="income">{data.map((_, i) => <Cell key={i} fill={palette.positive}/>)}</Bar></BarChart></ResponsiveContainer></div>;
}
